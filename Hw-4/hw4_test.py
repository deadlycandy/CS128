...
networkName = "mynetHome" # the name of the network you created
...
##########################################################################
    ...
    def test_e_shard_remove_node(self):
        ipPort = self.view[0]["testScriptAddress"]
        removedNode1 = self.view.pop()["networkIpPortAddress"]

        initialShardIDs = self.checkGetAllShardIds(ipPort)

        self.confirmDeleteNode(ipPort=ipPort,
                               removedAddress=removedNode1,
                               expectedStatus=200,
                               expectedResult="Success",
                               expectedMsg="Successfully removed %s from view"%removedNode1)
